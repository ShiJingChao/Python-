"""
    击中飞机:
        ...
        1. 敌方子弹击中我方飞机 (我方飞机类);
            ...
            def hit(self, bullet_list):   # bullet_list --> 敌方子弹列表
        ...
        2. 我方子弹击中敌方飞机 (敌方飞机类);
            ...
            def hit(self, bullet_list):   #  bullet_list ---> 我方子弹列表
                for bullet in bullet_list:
                    ----判断坐标-----
                    击中 --> break --> 爆炸


        * 判断某一个飞机是否被击中, 如果该飞机被击中, 让此飞机爆炸;
        我方飞机, 敌方飞机;
"""
...
class BasePlane:
    def __init__(self, x, y, img_path, screen):  # 显示的窗口
        ...
        self.bullet_list = []
        self.is_over = False  # 用来记录击中状态的属性, 该属性值, 初始为False, 代表没有被击中
    ...
    # 功能: 判断飞机是否被击中, 如果击中,返回True, 否则返回False
...
def main():
    ...
    while True:
        ...
        if hero.is_over:  # 判断我方飞机是否被击中
            if a <= 10:
                hero.img = pygame.image.load(r"..\feiji\hero_blowup_n1.png")
            elif a <= 20:
                hero.img = pygame.image.load(r"..\feiji\hero_blowup_n2.png")
            elif a <= 30:
                hero.img = pygame.image.load(r"..\feiji\hero_blowup_n3.png")
            elif a <= 40:
                hero.img = pygame.image.load(r"..\feiji\hero_blowup_n4.png")
            elif a == 50:
                break
            a += 1
        ...
        if enemy.is_over:  # 用记录击中状态的属性判断敌机是否被击中
            if b <= 10:
                enemy.img = pygame.image.load(r'..\feiji\enemy0_down1.png')
            elif b <= 20:
                enemy.img = pygame.image.load(r'..\feiji\enemy0_down2.png')
            elif b <= 30:
                enemy.img = pygame.image.load(r'..\feiji\enemy0_down3.png')
            elif b < 35:
                enemy.img = pygame.image.load(r'..\feiji\enemy0_down4.png')
            elif b == 35:
